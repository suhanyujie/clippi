...
- 标签进入分类栏：加进分类栏的标签显示在类型按钮之后，方向键一并循环。分类栏和左侧标签栏各有各的清单 —— 一个标签可以只在其中一处，从旧版本升级时会把原有的侧栏置顶复制一份到分类栏
- 分类栏单选：同一时刻只有一个分类按钮处于选中；再点一次已选中的按钮回到「全部」。点击和方向键走同一条路径，两者对「当前在哪个分类」的理解一致（Quick Paste 上的分类栏同理）
- 定制分类栏：右键分类栏打开配置面板，类型可显示/隐藏并排序，标签勾选即置顶/取消，同样可排序 —— 内置类型是固定的 8 种，想加一格新分类就建个标签置顶它
- 关键词搜索 — 同时匹配文本内容和标签名
